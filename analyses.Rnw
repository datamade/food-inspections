...
inspections$inspection_type == as.factor(tolower(inspections$inspection_type))

inspections <- inspections[!tolower(inspections$inspection_type) 
                           %in% c("no entry", 
                                  "non-inspection", 
...
non.high.risk <- unique(inspections[inspections$risk 
                                    %in% c("Risk 2 (Medium)", 
                                           "Risk 3 (Low)"), 
                                    "license_no"])
...
inspection.types <- table(inspections$inspection_type)
...
riskTime <- polr(results ~ scale(ivt) 
                 + I(scale(ivt)^2) 
                 + I(scale(ivt)^3)
                 + inspection_type
                 , 
...
<<riskInspectionType, results=tex, cache=F>>=
riskInspectionType <- polr(results ~ inspection_type, 
                           real.evaluations)



canvass.fail <- expit(riskInspectionType$zeta[1])
canvass.cond.pass <- expit(riskInspectionType$zeta[2]) - canvass.fail
canvass.pass <- 1 - canvass.fail - canvass.cond.pass

complaint.fail <- expit(riskInspectionType$zeta[1] 
                        - coef(riskInspectionType)[1])
complaint.cond.pass <- (expit(riskInspectionType$zeta[2] 
                              - coef(riskInspectionType)[1])
                        - complaint.fail)
complaint.pass <- 1 - complaint.fail - complaint.cond.pass

sf.complaint.fail <- expit(riskInspectionType$zeta[1] 
                        - coef(riskInspectionType)[2])
sf.complaint.cond.pass <- (expit(riskInspectionType$zeta[2] 
                              - coef(riskInspectionType)[2])
                        - sf.complaint.fail)
...
low.risk <- unique(inspections[inspections$risk == "Risk 3 (Low)", 
                               "license_no"])

med.risk <- unique(inspections[inspections$risk == "Risk 2 (Medium)", 
                               "license_no"])

high.risk <- unique(inspections[inspections$risk == "Risk 1 (High)", 
                                "license_no"])

multi.risk <- unique(intersect(low.risk, med.risk),
                     intersect(low.risk, high.risk),
                     intersect(med.risk, high.risk))

facilities <- unique(inspections[!inspections$license_no %in% multi.risk &
                                 inspections$risk %in% c("Risk 1 (High)", 
                                                         "Risk 2 (Medium)",
                                                         "Risk 3 (Low)"),
                                 c("license_no", "facility_type", "risk")])
...
clean.unique.facilities <- dim(facilities)[1]

facilities <- facilities[facilities$facility_type %in% top.facilities,]

facility.names <- c("restaurant",
                   "grocery store",
                    "unlabeled",
                    "school",
                    "daycare, all types",
...
facilityCount <- table(facilities$facility_type)
facilityCount <- facilityCount[order(-facilityCount)]
facilityCount <- cbind(facilityCount, facilityCount/clean.unique.facilities)
facilityCount <- rbind(facilityCount, colSums(facilityCount))

rownames(facilityCount) <- c(facility.names, "Total")
colnames(facilityCount) <- c("Count", "Proportion")

fc.tab <- xtable(facilityCount, digits=c(0,0,2))
...
riskFacilities <- table(facilities$facility_type, 
                        facilities$risk)

riskFacilities <- riskFacilities[order(-table(facilities$facility_type)),]
names(riskFacilities) <- facility.names

xtable(riskFacilities)
@ 

<<proportions, results=tex, cache=F>>=
xtable(prop.table(riskFacilities, 1))
@ 
